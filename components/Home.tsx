...
import { Flame, Map as MapIcon, ClipboardList, GraduationCap, Users, ArrowRight, ShieldCheck, Activity, LogOut } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Card 5: Logout (New Prominent Exit) */}
        <div
          onClick={onLogout}
          className="bg-slate-800 p-6 rounded-xl border border-slate-700 flex items-center justify-between cursor-pointer hover:bg-slate-700 hover:border-red-500/50 transition group"
        >
          <div>
            <h4 className="font-bold text-white text-sm uppercase tracking-wide mb-1 group-hover:text-red-400 transition">Disconnessione</h4>
            <p className="text-slate-400 font-medium text-sm flex items-center gap-1 group-hover:text-slate-300">
              Esci dall'applicazione
            </p>
          </div>
          <LogOut className="w-8 h-8 text-slate-500 group-hover:text-red-500 transition" />
        </div>
      </div>
    </div>
  );
